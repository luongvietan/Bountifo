...
export function renderRadarCsv(data: RadarExportData): string {
  // UTF-8 BOM so spreadsheet apps decode non-ASCII program names correctly.
  const lines = [String.fromCodePoint(0xfeff) + CSV_COLUMNS.join(",")];
...
/** Untrusted text in a block-level position (headings, bullets, quotes):
 *  escapeMd handles metacharacters but not line breaks — a `\n` inside a
 *  persisted name/warning could inject a new heading or list. Collapse the
 *  string to one line first, then escape. Table cells are handled by
 *  mdTable's own `<br>` conversion; this helper covers everywhere else. */
function inlineMd(text: string): string {
  return escapeMd(text.replace(/[\r\n]+/g, " "));
}

/** Persisted values we format ourselves (ISO timestamps): no metachar
 *  escaping — only the line-break injection vector needs neutralizing. */
...
function programCell(row: RadarExportRow): string {
  const label = inlineMd(row.name ?? row.slug);
...
function detailBlock(row: RadarExportRow): string {
  ...
  const lines: string[] = [
    `#### ${row.rank}. ${inlineMd(row.name ?? row.slug)} — evidence detail`,
    "",
...
export function renderRadarMarkdown(
  ...
  const lines: string[] = [
    ...
    `- Scan date: ${singleLine(run.started_at)} · run ${codeSpan(run.run_id)} · phase ${codeSpan(run.phase)}`,
  ...
  if (data.restricted_access.count > 0) {
    lines.push(
      ...
      `> **Notice:** this report contains information from ` +
        ...
        data.restricted_access.programs.map(inlineMd).join(", ") +
  ...
  lines.push(
    ...
    "## Executive summary",
    "",
    `- Run ID: ${codeSpan(run.run_id)} · scan date ${singleLine(run.started_at)} · updated ${singleLine(run.updated_at)}`,
      ...
      : `- Deep analysis: ${run.deep_candidates} candidate(s) · ` +
        ...
        (run.deep_stabilization === null
          ...
          : run.deep_stabilization === "stable"
            ...
            : inlineMd(run.deep_stabilization)),
  );
  // Score rows carry no run_id — for a run that never reached a terminal
  // verdict the tables can include evidence persisted by earlier scans
  // (pending uuids already have prior-run score rows). Say so honestly.
  if (run.status === null) {
    lines.push(
      "- Note: this run has no terminal verdict — rows may include evidence written by earlier scans; re-export after completion for a single-scan snapshot.",
    );
  }
  ...
  for (const section of data.sections) {
    lines.push(
      "",
      `## ${inlineMd(section.profile_label)} — ${codeSpan(section.profile_id)} v${singleLine(section.profile_version)}`,
      "",
  ...
  if (run.warning_details.length > 0) {
    const bullets = run.warning_details.map((w) => `- ${inlineMd(w)}`);
    // warning_details is the capped excerpt; disclose the truncation.
    if (run.warnings > run.warning_details.length) {
      bullets.push(
        `- …and ${run.warnings - run.warning_details.length} more (list capped)`,
      );
    }
    lines.push("", "### Scan warnings", "", bullets.join("\n"));
  }
  return `${lines.join("\n")}\n`;
}
 ...
 *  The hash covers the canonical pre-redaction data model — when a stored
 *  credential ever lands in the body it is scrubbed AFTER hashing, so a
 *  redacted body's embedded content_sha256 reflects the unredacted model.
 *  That is deliberate: the hash identifies the scan data, never the
 *  (impossible-to-publish) secret-bearing bytes. */
